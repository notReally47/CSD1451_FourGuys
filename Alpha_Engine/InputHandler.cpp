...
namespace InputHandler {
	...
	bool buttonClick(float buttonX, float buttonY) {
		...
		if (AEInputCheckReleased(AEVK_LBUTTON)&& mouseX <=static_cast<s32>(AEGetWindowWidth() / 2 + (buttonX + 50.f)) && mouseX >= static_cast<s32>(AEGetWindowWidth() / 2 + (buttonX - 50.f))
		&& mouseY <= static_cast<s32>(AEGetWindowHeight() / 2 + (buttonY + 25.f)) && mouseY >= static_cast<s32>(AEGetWindowHeight() / 2 + (buttonY - 25.f)))
			return true;
		return false;
	}
}